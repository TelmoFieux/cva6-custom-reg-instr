...
#(
  ...
  // same behavior as load_unit mux
  function automatic logic [CVA6Cfg.XLEN-1:0] ld_extend(fu_op op, logic [CVA6Cfg.XLEN-1:0] d);
    logic [CVA6Cfg.XLEN-1:0] r;
    r = d;
    unique case (op)
      // word
      LW, HLV_W:              begin r = {CVA6Cfg.XLEN{d[31]}}; r[31:0] = d[31:0]; end
      LWU, HLV_WU, HLVX_WU:   begin r = '0;                    r[31:0] = d[31:0]; end
      // half word
      LH, HLV_H:              begin r = {CVA6Cfg.XLEN{d[15]}}; r[15:0] = d[15:0]; end
      LHU, HLV_HU, HLVX_HU:   begin r = '0;                    r[15:0] = d[15:0]; end
      // byte
      LB, HLV_B:              begin r = {CVA6Cfg.XLEN{d[7]}};  r[7:0]  = d[7:0];  end
      LBU, HLV_BU:            begin r = '0;                    r[7:0]  = d[7:0];  end
      default: begin
        if (CVA6Cfg.FpPresent) begin
          unique case (op)
            FLW:     begin r = '1; r[31:0] = d[31:0]; end
            FLH:     begin r = '1; r[15:0] = d[15:0]; end
            FLB:     begin r = '1; r[7:0]  = d[7:0];  end
            default: r = d;
          endcase
        end
      end
  ...
  typedef struct packed {
    ...
    logic [LSQ_DEPTH-1:0] committed; // instr has been accepted by the store buffer
  ...
  typedef struct packed {
    ...
    logic [LSQ_DEPTH-1:0] ready; // data and paddr are valid
    logic [LSQ_DEPTH-1:0] reserved; // instr has been accepted by the rob
    logic [LSQ_DEPTH-1:0] issued; // instr has been issued to the cache
    logic [LSQ_DEPTH-1:0][CVA6Cfg.TRANS_ID_BITS-1:0] vaddr_trans_id;
    logic [LSQ_DEPTH-1:0] vaddr_valid; // vaddr is valid
    logic [LSQ_DEPTH-1:0][CVA6Cfg.PLEN-1:0] paddr;
    logic [LSQ_DEPTH-1:0] paddr_valid; // vaddr has been translated
  ...
  //TODO : remember to modify this if ld_queue size != st_queue_size
  localparam AGE_MATRIX_SIZE = LSQ_DEPTH*2;
  ...
  // Age Matrix
  logic [AGE_MATRIX_SIZE-1:0][AGE_MATRIX_SIZE-1:0] age_matrix_n, age_matrix_q;
  ...
  // store matching and older
  logic [LSQ_DEPTH-1:0][LSQ_DEPTH-1:0] full_match_candidate;
  logic [LSQ_DEPTH-1:0][LSQ_DEPTH-1:0] partial_match_candidate;
  ...
  logic [LSQ_DEPTH-1:0] data_valid, fwd_data_valid;
  ...
  assign ld_ready_pointer_valid = !ld_ready_empty;
  ...
  always_comb begin : st_full_logic

    automatic int unsigned pending_stores;

    pending_stores = st_status_cnt_q;

    for (int unsigned i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin

      st_full[i] = st_we_i[i] && (pending_stores >= LSQ_DEPTH);

      if (st_we_i[i] && !st_full[i]) begin
        pending_stores++;
      end
    end
  end
  ...
  for (genvar i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin : ld_alloc_tree
      ...
      assign ld_free_entries_masked[i+1] = (ld_we_i[i] && ld_empty_mask[i] == 1'b0) ?
  ...
  logic [AGE_MATRIX_SIZE-1:0] translation_req;

  logic [AGE_MATRIX_SIZE-1:0] oldest_ready;
  logic [$clog2(AGE_MATRIX_SIZE)-1:0] oldest_idx;
  logic oldest_empty;

  assign translation_req = {st_comb_vaddr_valid, ld_comb_vaddr_valid};

  for (genvar i = 0; i < AGE_MATRIX_SIZE; i++) begin
      assign oldest_ready[i] = translation_req[i] && !(|(age_matrix_n[i] & translation_req));
  end

  lzc #(
      .WIDTH(AGE_MATRIX_SIZE),
      .MODE(1'b0))
  i_translation_ready_lzc (
      .in_i   (oldest_ready),
      .cnt_o  (oldest_idx),
      .empty_o(oldest_empty)
  );


  always_comb begin : age_matrix_update

    automatic logic [AGE_MATRIX_SIZE-1:0] current_state;
    automatic int unsigned idx;

    age_matrix_n = age_matrix_q;

    current_state = {st_queue_q.reserved, ld_queue_q.reserved};

    for (int unsigned i = 0; i<CVA6Cfg.NrIssuePorts ; i++ ) begin

      if (ld_we_i[i] && !ld_full[i]) begin
        idx = ld_alloc_idx[i];

        for (int unsigned k = 0 ; k < AGE_MATRIX_SIZE ; k++) begin
          age_matrix_n[k][idx] = 1'b0;
        end

        age_matrix_n[idx] = current_state;
        current_state[idx] = 1'b1;
      end

      if (st_we_i[i] && !st_full[i]) begin
        idx = st_alloc_idx[i] + LSQ_DEPTH;
  ...
  assign ld_wb_pointer_valid = !ld_wb_empty;

  logic [LSQ_DEPTH-1:0][CVA6Cfg.XLEN-1:0] fwd_data_snooped;
  logic [LSQ_DEPTH-1:0] data_snooped_valid, fwd_data_snooped_valid;
  logic [LSQ_DEPTH-1:0] ld_is_forwarded;

  logic [$clog2(LSQ_DEPTH)-1:0] ld_ready_pointer_n, ld_ready_pointer_q;
  logic ld_ready_pointer_valid_n, ld_ready_pointer_valid_q;
  ...
  always_comb begin : updating_queues
    ...
    for (int i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
      if (ld_we_i[i] & !ld_full[i]) begin
        ...
        ld_queue_n.issued[ld_alloc_idx[i]] = 1'b0;
        ld_queue_n.instr[ld_alloc_idx[i]] = decoded_req[i];
        ld_queue_n.ready[ld_alloc_idx[i]] = '0;
        ld_queue_n.ex[ld_alloc_idx[i]] = '0;
        ld_queue_n.paddr_valid[ld_alloc_idx[i]] = '0;
        ld_queue_n.result_valid[ld_alloc_idx[i]] = '0;
        ld_queue_n.paddr_ni[ld_alloc_idx[i]] = '0;
    ...
    for (int unsigned i = 0 ; i < CVA6Cfg.NrIssuePorts ; i++) begin
      if (st_we_i[i] && !st_full[i]) begin
        ...
        st_queue_n.paddr_valid[st_issue_pointer] = '0;
        st_queue_n.committed[st_issue_pointer] = 1'b0;
    ...
    for ( int unsigned i = 0 ; i < LSQ_DEPTH ; i++) begin
      for (int unsigned j = 0 ; j < CVA6Cfg.NrWbPorts ; j++) begin
        if (wt_valid_i[j] && wb_trans_id_i[j] == st_queue_q.data_trans_id[i] && st_queue_q.reserved[i] && !st_queue_q.data_valid[i]) begin
          ...
          //prevent same cycle forwarding only for loads
          //required to avoid combinatorial loop when load_pipe_reg = 0 in lsu
          if (j != LOAD_WB) begin
            fwd_data_snooped_valid[i] = 1'b1;
            fwd_data_snooped[i] = wbdata_i[j];
          end
    ...
    // issues
    // TODO: issues with timing but retesting
    if (CVA6Cfg.MmuPresent && CVA6Cfg.NonIdemPotenceEn && dtlb_hit_i && comb_translation_pointer_type == 1'b0) begin
      if (dtlb_hit_i) begin
    ...
    end else if (CVA6Cfg.NonIdemPotenceEn & comb_translation_pointer_valid & comb_translation_pointer_type == 1'b0) begin
      ld_queue_n.paddr_ni[comb_translation_pointer] = config_pkg::is_inside_nonidempotent_regions
        (
          CVA6Cfg,
          {{52 - CVA6Cfg.PPNW{1'b0}}, dtlb_ppn_i, 12'd0}
    ...
    for (int unsigned i = 0; i<LSQ_DEPTH; i ++) begin
      if(comb_st_paddr_valid[i] && data_valid[i] && st_queue_q.reserved[i] & !comb_st_ex_valid[i]) begin
        st_queue_n.ready[i] = 1'b1;
      end

      // load is not ready if :
      // - all previous store did not receive their paddr (we cannot verify dependencies)
      // - it is a non idempotent load and not all of the previous instr have been committed or
      ...
      if(comb_ld_paddr_valid[i] && ld_queue_q.reserved[i] && !ld_queue_q.result_valid[i] && !comb_ld_ex_valid[i] && !ld_queue_q.issued[i]) begin
        ld_queue_n.ready[i] = 1'b1;
        ld_unit_ready[i] = 1'b1;
        if (ld_queue_q.paddr_ni[i]) begin
          ld_queue_n.ready[i] = commit_trans_id_i == ld_queue_q.instr[i].trans_id & dcache_wbuffer_not_ni_i & no_st_pending_i;
          ld_unit_ready[i] = ld_queue_q.ready[i];
        end else begin
          if (|st_older[i]) begin
            if (!st_older_paddr_valid[i]) begin
              ...
              for (int unsigned j = 0; j<LSQ_DEPTH; j ++) begin
                if(st_older[i][j] && (comb_matching_addr[i][j] || comb_partial_matching_addr[i][j])) begin
                  ld_queue_n.ready[i] = 1'b0;
                  ld_unit_ready[i] = 1'b0;
                end
              end
      ...
      // never flag ready an instrucion with a result valid or exception
    ...
    for (int unsigned i = 0; i<LSQ_DEPTH; i ++) begin
      if(comb_ld_paddr_valid[i] & ld_queue_q.reserved[i] & !ld_queue_q.paddr_ni[i] & !comb_ld_ex_valid[i]) begin
        // only forward if all data is available
        if (st_older_all_valid[i]) begin
          if (!full_no_match[i]) begin
            if (!partial_no_match[i]) begin
              if (age_matrix_q[full_match_winner_idx[i]+LSQ_DEPTH][partial_match_winner_idx[i]+LSQ_DEPTH]) begin
                ld_queue_n.result_valid[i] = 1'b1;
                ld_queue_n.result[i] = ld_extend(ld_queue_q.instr[i].operation, fwd_data[full_match_winner_idx[i]]);
    ...
    if (st_commit_fire) begin
      ...
      st_queue_n.committed[st_commit_pointer_q] = 1'b1;
    end

    if(st_sent_to_cache_q) begin
      ...
      st_queue_n.committed[st_drain_pointer_q] = '0;
      st_status_cnt = st_status_cnt - 1'b1;
      st_return_token = st_return_token + 1'b1;
    end


    ld_ready_pointer_n = ld_ready_pointer_q;
    ld_ready_pointer_valid_n = ld_ready_pointer_valid_q;

    //load unit
    if (ld_pop_i) begin
      ld_ready_pointer_valid_n = '0;
      if (ld_ready_pointer_valid_q) begin
        ld_queue_n.ready[ld_ready_pointer_q] = 1'b0;
        ld_queue_n.issued[ld_ready_pointer_q] = 1'b1;
      end else begin
    ...
    for (int unsigned i = 0; i < LSQ_DEPTH; i++) begin
      for (int unsigned j = 0 ; j<CVA6Cfg.RollbackWidth ; j++) begin
        if (ld_queue_n.reserved[i] && ld_queue_n.instr[i].trans_id == rollback_trans_id_i[j] && rollback_i[j]) begin
          ld_queue_n.reserved[i] = 1'b0;
          ld_queue_n.ready[i] = 1'b0;
          ld_queue_n.issued[i] = 1'b0;
        ...
        if (st_queue_n.reserved[i] && !st_queue_n.committed[i] && st_queue_n.instr[i].trans_id == rollback_trans_id_i[j] && rollback_i[j]) begin
          ...
          st_queue_n.committed[i] = '0;
          st_status_cnt = st_status_cnt - 1'b1;
          st_issue_pointer = st_issue_pointer -1'b1;
          st_return_token = st_return_token + 1'b1;
        end

        if (ld_ready_pointer_valid_q && ld_queue_q.instr[ld_ready_pointer_q].trans_id == rollback_trans_id_i[j] && rollback_i[j]) begin
          ld_ready_pointer_valid_n = '0;
        end
    ...
    if (flush_i) begin
      ...
      for (int unsigned i = 0; i < LSQ_DEPTH; i++) begin
        if (st_queue_n.reserved[i] && !st_queue_n.committed[i]) begin
          st_queue_n.reserved[i]    = 1'b0;
          st_queue_n.ready[i]       = 1'b0;
          st_queue_n.vaddr_valid[i] = 1'b0;
          st_queue_n.paddr_valid[i] = 1'b0;
          st_queue_n.data_valid[i]  = 1'b0;
          st_queue_n.ex_valid[i]    = 1'b0;
          st_queue_n.committed[i]   = 1'b0;
          st_return_token = st_return_token + 1'b1;
        end
      end

      st_issue_pointer = st_commit_pointer_n;

      st_status_cnt = '0;
      for (int unsigned i = 0; i < LSQ_DEPTH; i++) begin
        if (st_queue_n.reserved[i]) begin
          st_status_cnt++;
        end
      end
    end
  ...
  always_comb begin : store_is_rollbacked
    st_rollbacked = 1'b0;

    for (int unsigned i = 0; i < CVA6Cfg.RollbackWidth; i++) begin
      if (rollback_i[i] && st_queue_q.reserved[st_commit_pointer_q] && !st_queue_q.committed[st_commit_pointer_q]
        && st_queue_q.instr[st_commit_pointer_q].trans_id == rollback_trans_id_i[i])
      begin
        st_rollbacked = 1'b1;
      end
    end
  ...
  assign translation_type_o = (CVA6Cfg.MmuPresent && translation_pointer_valid_q) ? translation_pointer_type_q : comb_translation_pointer_type;
  ...
  assign translation_req_int = (CVA6Cfg.MmuPresent ?
      (translation_pointer_valid_q || comb_translation_pointer_valid)
    : comb_translation_pointer_valid);
 assign translation_req_o = translation_req_int && !flush_i;
  ...
  always_comb begin : comb_translation_data
    ...
    for (int unsigned i = 0; i < LSQ_DEPTH; i++) begin
      st_is_translating[i] = (CVA6Cfg.MmuPresent && translation_pointer_valid_q && translation_pointer_type_q == 1'b1 && translation_pointer_q == i)
          || (!CVA6Cfg.MmuPresent && CVA6Cfg.NonIdemPotenceEn && translation_data_valid_q && previous_translation_pointer_type_q == 1'b1 && previous_translation_pointer_q == i);
      ld_is_translating[i] = (CVA6Cfg.MmuPresent && translation_pointer_valid_q && translation_pointer_type_q == 1'b0 && translation_pointer_q == i)
          || (!CVA6Cfg.MmuPresent && CVA6Cfg.NonIdemPotenceEn && translation_data_valid_q && previous_translation_pointer_type_q == 1'b0 && previous_translation_pointer_q == i);
    end
    ...
    for (int unsigned i = 0 ; i < LSQ_DEPTH ; i++) begin
      ...
      if (st_queue_q.reserved[i] && st_queue_q.vaddr_valid[i] && !st_queue_q.paddr_valid[i] && !st_is_translating[i]) begin
      ...
      // validity updated from st_queue_q and current paddr state
      if (ld_queue_q.reserved[i] && ld_queue_q.vaddr_valid[i] && !ld_queue_q.paddr_valid[i] && !ld_is_translating[i]) begin
  ...
  always_comb begin : updating_translation_pointer
    ...
    if (CVA6Cfg.MmuPresent) begin
      ...
      if (!translation_pointer_valid_q || dtlb_hit_i) begin
        ...
        if(!oldest_empty) begin
          comb_translation_pointer = oldest_idx[$clog2(LSQ_DEPTH)-1:0];
          comb_translation_pointer_type =  oldest_idx[$clog2(AGE_MATRIX_SIZE)-1];
          comb_translation_pointer_valid = 1'b1;

          if (!dtlb_hit_i) begin
            translation_pointer_n = oldest_idx[$clog2(LSQ_DEPTH)-1:0];
            translation_pointer_type_n =  oldest_idx[$clog2(AGE_MATRIX_SIZE)-1];
            translation_pointer_valid_n = 1'b1;
          end
    ...
    // previous one is sent
      ...
      translation_data_valid_n = translation_req_int;
    ...
    for (int unsigned i = 0 ; i<CVA6Cfg.RollbackWidth ; i++) begin
      ...
      if (rollback_i[i] && translation_req_int) begin
  ...
  assign ld_unit_paddr_o = comb_ld_paddr[ld_selected_pointer];
  ...
  always_comb begin : adress_dependencies
    ...
    comb_ld_paddr_valid = ld_queue_q.paddr_valid;
    comb_ld_paddr = ld_queue_q.paddr;
    comb_st_paddr_valid = st_queue_q.paddr_valid;
    comb_st_paddr = st_queue_q.paddr;
    ...
    if (translation_data_valid_q) begin
      if (previous_translation_pointer_type_q == 1'b0) begin
        ...
        comb_st_paddr_valid[previous_translation_pointer_q] = 1'b1;
        comb_st_paddr[previous_translation_pointer_q] = paddr_i;
      end
    end
  ...
  always_comb begin : adress_match
    ...
    comb_matching_addr = ld_queue_q.matching_addr;
    comb_partial_matching_addr = ld_queue_q.partial_matching_addr;

    if (previous_translation_pointer_type_q == 1'b0 & comb_ld_paddr_valid[previous_translation_pointer_q]) begin
      for (int unsigned j = 0; j<LSQ_DEPTH; j ++) begin
        if(comb_st_paddr_valid[j]) begin
          if (comb_ld_paddr[previous_translation_pointer_q] == comb_st_paddr[j] && st_queue_q.instr[j].be == ld_queue_q.instr[previous_translation_pointer_q].be) begin
            comb_matching_addr[previous_translation_pointer_q][j] = 1'b1;
          end else if (overlap_check(ld_queue_q.instr[previous_translation_pointer_q].be, comb_ld_paddr[previous_translation_pointer_q], st_queue_q.instr[j].be, comb_st_paddr[j])) begin
            comb_partial_matching_addr[previous_translation_pointer_q][j] = 1'b1;
          end
        end else begin
          comb_matching_addr[previous_translation_pointer_q][j] = 1'b0;
          comb_partial_matching_addr[previous_translation_pointer_q][j] = 1'b0;
        end
      end
    end else if (previous_translation_pointer_type_q == 1'b1 & comb_st_paddr_valid[previous_translation_pointer_q]) begin
      for (int unsigned i = 0; i<LSQ_DEPTH; i ++) begin
        if(comb_ld_paddr_valid[i]) begin
          if (comb_ld_paddr[i] == comb_st_paddr[previous_translation_pointer_q] && st_queue_q.instr[previous_translation_pointer_q].be == ld_queue_q.instr[i].be) begin
            comb_matching_addr[i][previous_translation_pointer_q] = 1'b1;
          end else if (overlap_check(ld_queue_q.instr[i].be, comb_ld_paddr[i], st_queue_q.instr[previous_translation_pointer_q].be, comb_st_paddr[previous_translation_pointer_q])) begin
            comb_partial_matching_addr[i][previous_translation_pointer_q] = 1'b1;
          end
        end else begin
          comb_matching_addr[i][previous_translation_pointer_q] = 1'b0;
          comb_partial_matching_addr[i][previous_translation_pointer_q] = 1'b0;
        end
      end
    end

    if (!(CVA6Cfg.MmuPresent || CVA6Cfg.NonIdemPotenceEn)) begin
      for (int unsigned i = 0; i<LSQ_DEPTH; i ++) begin
        if(comb_ld_paddr_valid[i]) begin
          for (int unsigned j = 0; j<LSQ_DEPTH; j ++) begin
            if(comb_st_paddr_valid[j]) begin
              if (comb_ld_paddr[i] == comb_st_paddr[j] && st_queue_q.instr[j].be == ld_queue_q.instr[i].be) begin
                comb_matching_addr[i][j] = 1'b1;
              end else if (overlap_check(ld_queue_q.instr[i].be, comb_ld_paddr[i], st_queue_q.instr[j].be, comb_st_paddr[j])) begin
  ...
  // match with older store
  for (genvar i = 0; i < LSQ_DEPTH; i++) begin
    assign full_match_candidate[i] = comb_matching_addr[i] & st_older[i];
    assign partial_match_candidate[i] = comb_partial_matching_addr[i] & st_older[i];
  end

  for (genvar i = 0 ; i < LSQ_DEPTH ; i++) begin
    assign full_match_rotated_mask[i] = {full_match_candidate[i], full_match_candidate[i]} >> st_drain_pointer_q;
    assign partial_match_rotated_mask[i] = {partial_match_candidate[i], partial_match_candidate[i]} >> st_drain_pointer_q;
  end
  ...
  for (genvar i = 0 ; i < LSQ_DEPTH ; i++) begin
    ...
    assign full_match_winner_idx[i] = st_drain_pointer_q + (LSQ_DEPTH - 1 - full_match_winner_k[i]);
    assign partial_match_winner_idx[i] = st_drain_pointer_q + ( LSQ_DEPTH - 1 - partial_match_winner_k[i]);
  end
  ...
  always_comb begin : forwarding_readyness_data
    ...
    for (int unsigned i = 0; i<LSQ_DEPTH; i ++) begin
      ...
      fwd_data_valid[i] = st_queue_q.data_valid[i] || fwd_data_snooped_valid[i];
      fwd_data[i] = st_queue_q.data_valid[i] ? st_queue_q.instr[i].data : fwd_data_snooped[i];
    end
    ...
    for (int unsigned i = 0; i<LSQ_DEPTH; i ++) begin
      for (int unsigned j = 0; j<LSQ_DEPTH; j ++) begin
        if (age_matrix_q[i][j+LSQ_DEPTH] && st_queue_q.reserved[j] & !st_queue_q.ex_valid[j]) begin
          ...
          if (!fwd_data_valid[j]) st_older_all_valid[i] = 1'b0;
        end
      end
    end
  ...
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
      ...
      ld_ready_pointer_q <= ld_ready_pointer_n;
      ld_ready_pointer_valid_q <= ld_ready_pointer_valid_n;
    end
  end
  ...
  st_sent_to_cache_i
  |->
  st_queue_q.reserved[st_drain_pointer_q] &&
  st_queue_q.committed[st_drain_pointer_q]
  );

  for (genvar i = 0; i < LSQ_DEPTH; i++) begin
  assert property (
    @(posedge clk_i) disable iff (!rst_ni)
    st_queue_q.committed[i]
    |->
    st_queue_q.reserved[i]
  );
  end
  ...
  assert property (
    ...
    (
      commit_trans_id_i ==
        ld_queue_q.instr[ld_selected_pointer].trans_id
      &&
      no_st_pending_i
      &&
    ...
    $error(
      "ILLEGAL NI LOAD: idx=%0d tid=%0d commit_tid=%0d stbuf_empty=%b dcache_wb_safe=%b paddr=%h",
      $sampled(ld_selected_pointer),
      $sampled(ld_queue_q.instr[ld_selected_pointer].trans_id),
      $sampled(commit_trans_id_i),
      $sampled(no_st_pending_i),
      $sampled(dcache_wbuffer_not_ni_i),
      $sampled(comb_ld_paddr[ld_selected_pointer])
    );
  end
  ...
  assert property (
    ...
    $error(
      "NI LOAD ACCEPTED SAME CYCLE AS STORE COMMIT: ld_tid=%0d st_tid=%0d",
      $sampled(ld_queue_q.instr[ld_selected_pointer].trans_id),
      $sampled(st_queue_q.instr[st_commit_pointer_q].trans_id)
    );
  end
  ...
  for (genvar i = 0; i < CVA6Cfg.NrIssuePorts; i++) begin
    assert property (
      ...
      st_we_i[i] && !st_full[i]
      |->
      !st_queue_q.reserved[st_alloc_idx[i]]
    )
    else $error(
      "SQ ALLOC ON RESERVED ENTRY: port=%0d idx=%0d reserved=%b count=%0d",
      i,
      $sampled(st_alloc_idx[i]),
      $sampled(st_queue_q.reserved),
